# Memory Bridge — Codex Agent Instructions
...
When user says "save to memory bridge", "memory-bridge save", "save session", or "archive this":
...
4. **Write file** — `YYYYMMDD-HHMMSS-codex.md`
...
6. **Git workflow** — auto-commit and push:
   ```bash
   git add {filepath}
   git commit -m "memory(codex): {timestamp} - {brief-topic} [+artifact: {filename}]"
...
The Memory Bridge MCP server can run centrally for all agents. Codex connects via **Streamable HTTP**.

### Why Streamable HTTP?

Codex's MCP client uses the modern Streamable HTTP transport. The server provides:
- `:8080/sse` — Legacy SSE for older clients
- `:8081/mcp` — **Streamable HTTP for Codex**
...
After configuring, restart Codex and verify connection:
```bash
...
If Codex shows `405 Method Not Allowed`, it's trying to use SSE. Ensure your config uses:
- URL: `http://<YOUR_VPS_TAILSCALE_IP>:8081/mcp` (not `:8080/sse`)
- Codex MCP client will negotiate Streamable HTTP automatically

## Agent Identity

- agent_id: `codex`
- Filename: `YYYYMMDD-HHMMSS-codex.md`
- Include this file (`.codex/CODEX.md`) in context when working in this repo
