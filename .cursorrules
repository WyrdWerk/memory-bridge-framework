# Memory Bridge — Cursor Agent Instructions
...
4. **Write file** — `YYYYMMDD-HHMMSS-cursor.md`
...
6. **Git workflow** — auto-commit and push:
   ```bash
   git add {filepath}
   git commit -m "memory(cursor): {timestamp} - {brief-topic}"
   git push origin main
   ```
   
   **If artifact embedded:**
   ```bash
   git commit -m "memory(cursor): {timestamp} - {topic} [+artifact: {filename}]"
   ```
...
The Memory Bridge MCP server can run centrally (e.g., on a VPS) for all agents to connect to. Cursor connects via SSE or Streamable HTTP transport.
...
After configuring, restart Cursor and verify connection:
```bash
...
- agent_id: `cursor`
- Filename: `YYYYMMDD-HHMMSS-cursor.md`
- Auto-loads: This file (`.cursorrules`) is read automatically when the repo is opened in Cursor
